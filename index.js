...
if ($(window).width() < 1000)
	$("#loader > img").css("width", ($(window).width() * 95) / 100 + "px");
else $("#loader > img").css("width", ($(window).width() * 0) / 100 + "px");
var homed = "";
var aboutd = "";
var homev = 0;
var total = "17";
...
function next() {
	...
	if (c == listOriginal.length - 1) c = -1;
...
$(document).ready(function () {
	...
	for (i = 0; i < listOriginal.length; i++) {
		$("#list").append(
			'<div id="' +
				i +
				'" class="listitem" onmouseover="mouseover(this)" onmouseout="mouseout(this)" onclick="change(this);" ><img src="image/' +
				list[i].img +
				'.jpg" width="150"><p  id="' +
				i +
				'" class="text">' +
				(list[i].header == "" ? " " : "Episode " + list[i].header) +
...
$(window).ready(function () {
	...
	aboutd = "<div id='aboutdiv'>" + $("#aboutdiv").html();
	+"</div>";
	homev = 0;
});

$(window).resize(function () {
	if ($(window).width() < 1000) $("#playing").css("bottom", "18%");
	else $("#playing").css("bottom", "25%");
});
